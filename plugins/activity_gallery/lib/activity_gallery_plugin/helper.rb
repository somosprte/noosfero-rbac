module ActivityGalleryPlugin::Helper
  ...
  def display_submissions(activity_gallery, user)
    return if activity_gallery.submissions.empty?
    content_tag('table',
      content_tag('tr',
      ...
      activity_gallery.children.order('name ASC').map {|author_folder| display_author_folder(author_folder, user)}.join("\n").html_safe
    )
  end
  ...
  def link_to_submission(submission, user)
    if ActivityGalleryPlugin.can_download_submission?(user, submission)
  ...
  def link_to_last_submission(author_folder, user)
    if ActivityGalleryPlugin.can_download_submission?(user, author_folder.children.last)
  ...
  def display_privacy_button(author_folder, user)
    ...
    activity_gallery = folder.parent
    @back_to = url_for(activity_gallery.url)

    if(user && activity_gallery.allow_visibility_edition &&
      ...
      button :edit, _('Edit'), { :controller => 'activity_gallery_plugin_myprofile',
